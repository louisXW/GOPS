...
class MPIController(Controller):
    ...
    def _submit_work(self, proposal):
        ...
        record = self.new_feval(proposal.args[0], extra_args=proposal.args[1])
...
class MPIControllerSync(MPIController):
    def __init__(self, strategy=None):
        "Initialize the controller."
        MPIController.__init__(self, strategy=None)
        self.runningworkers = []
        self.strategy = strategy
    ...
    def _handle_message(self):
        ...
        logger.debug("Handle incoming message")
        s = MPI.Status()
        logger.debug("after run the MPI status %s", s)
        data = comm.recv(status=s, source=MPI.ANY_SOURCE, tag=0)
        logger.debug("Received message: %s", data)
        mname = data[0]
        record = self._recids[data[1]]
        ...
        if mname == 'complete' or mname == 'cancel' or mname == 'kill':
            logger.debug("Re-queueing worker")
            self._workers.append(s.source)
            self.runningworkers.remove(s.source)
        elif mname == 'running':
            logger.debug("one worker is running")
            self.runningworkers.append(s.source)
        self.ping()
    ...
    def _run(self, merit=None, filterp=None):
        ...
        while True:
            if comm.Iprobe():
                logger.debug('running comm.Iprobe')
                self._handle_message()
            proposal = None
            logger.debug('The number os worker running:%d', len(self.runningworkers))
            if len(self.runningworkers) == self.strategy.nsamples*self.strategy.ncenters:
                logger.info("The workers that are running: %s", ",".join([str(w) for w in self.runningworkers]))
                sleep_time = 30
            else:
                sleep_time = 0
            if not self._needs_ping:
                logger.info("no need to self ping")
                proposal = self.strategy.propose_action()
            if not proposal:
                logger.debug('running after no proposal')
                if comm.Iprobe():
                    self._handle_message()
                else:

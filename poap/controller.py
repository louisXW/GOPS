...
class ThreadControllerReal(ThreadController):
    ...
    def _submit_work(self, proposal):
        ...
        try:
            worker = self.workers.get_nowait()
            logger.debug("Accept eval proposal")
            print ('_submit_work', worker.name)
            print ('proposal.args', proposal.args)
            proposal.record = self.new_feval(proposal.args[0], extra_args=proposal.args[1])
            proposal.record.worker = worker
            proposal.accept()
            worker.eval(proposal.record)
        except Queue.Empty:
